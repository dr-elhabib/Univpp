        ...
        public static  void  PrintFile(String s) {
            ...
            try
            {
               ...
               var Sheet = (Microsoft.Office.Interop.Excel.Worksheet)xlBook.Worksheets.get_Item("p"); // Explicit cast is not required here
                foreach (Worksheet ws in xlBook.Worksheets.OfType<Worksheet>())
                {
                    ws.PageSetup.Zoom = false;
                    ws.PageSetup.FitToPagesWide = 1;
                    ws.PageSetup.FitToPagesTall = false;
                    //ws.PageSetup.LeftMargin = App.InchesToPoints(0.7);
                    MessageBox.Show(ws.PageSetup.LeftMargin + "");
                   // ws.PageSetup.RightMargin = App.InchesToPoints(0.7);
                  ///  ws.PageSetup.TopMargin = App.InchesToPoints(0.75);
                   // ws.PageSetup.BottomMargin = App.InchesToPoints(0.75);
                  //  ws.PageSetup.HeaderMargin = App.InchesToPoints(0.3);
                //    ws.PageSetup.FooterMargin = App.InchesToPoints(0.3);
                ...
                // Print the range
                xlBook.PrintOutEx(misValue, misValue, misValue, misValue,
