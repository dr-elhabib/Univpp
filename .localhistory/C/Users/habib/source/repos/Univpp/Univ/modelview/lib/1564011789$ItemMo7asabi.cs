 ...
 public  class ItemMo7asabi : BaseViewModel
    {
        ...
        public ItemMo7asabi(card_mo7sabi card_mo7sabi)
        {
            ...
            remove = new Command(() => {
                ...
                ToList().Where(c => c.Id == card_mo7sabi.id_card).ToList().SingleOrDefault().id_prosess&&c.card.date> Ico.getValue<db>().GetUnivdb().cards.
                ToList().Where(c => c.Id == card_mo7sabi.id_card).ToList().SingleOrDefault().date)) {
