...
namespace Univ.modelview
{
    ...
    class AddpartCardViewModel : BaseViewModel<part>
    {
        ...
        public async Task Creat_card(part part)
        {
          await  Task.Run(() =>
            {
            var kanoni=   Ico.getValue<db>().GetUnivdb().parts.ToList().Where(p => p.Id == part.Id).ToList().FirstOrDefault().card_kanoni.ToList().FirstOrDefault();
                Ico.getValue<db>().GetUnivdb().card_kanoni.Remove(kanoni);
                Ico.getValue<db>().GetUnivdb().cards.Remove(kanoni.card);
