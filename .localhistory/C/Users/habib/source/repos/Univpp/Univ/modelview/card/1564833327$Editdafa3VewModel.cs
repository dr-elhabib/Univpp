...
namespace Univ.modelview
{
    class Editdafa3VewModel : BaseViewModel<card_dafa3>
    {
        ...
        public Editdafa3VewModel(card_dafa3 card_dafa3)
        {
            ...
            {
                if (Cost!=card_dafa3.Cost|| AlCost!=card_dafa3.alcost|| tswiya != card_dafa3.tswiya) { 
            ...
            Ico.getValue<ContentApp>().OpenSample4Dialog();
                    ...
                    else
                    {
                        ...
                        if ((d - d2) > Cost)
                        {
                            ...
                            Ico.getValue<db>().GetUnivdb().card_dafa3.ToList().Where(c => c.Id == card_dafa3.Id).FirstOrDefault().Cost = Cost;
                            ...
                            Ico.getValue<db>().GetUnivdb().parts.ToList().Where(c => c.Id == card_dafa3.id_part).FirstOrDefault().nowcost -= (card_dafa3.Cost - Cost);
                            Ico.getValue<db>().GetUnivdb().parts.ToList().Where(c => c.Id == card_dafa3.id_part).FirstOrDefault().mcost+= (card_dafa3.Cost - Cost);
