    ...
    class ItemPart : BaseViewModel<part>
    {
        ...
        public ItemPart(part part)
        {
            ...
            remove = new Command(() =>
                ...
                edit = new Command(() =>
                {
                var cardm = Ico.getValue<db>().GetUnivdb().card_mo7sabi.ToList().Where(c => c.id_part == part.Id).FirstOrDefault();
                var cardd = Ico.getValue<db>().GetUnivdb().card_dafa3.ToList().Where(c => c.id_part == part.Id).FirstOrDefault();
                    if (cardm == null&&cardd==null)
                    {
