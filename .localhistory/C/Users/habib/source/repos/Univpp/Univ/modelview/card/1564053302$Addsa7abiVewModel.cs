    ...
    class Addsa7abiVewModel : BaseViewModel
    {
        ...
        public Addsa7abiVewModel(process process)
        {
         ...
         //   this.date = Ico.getValue<Date>().GetPevDate().year1.Year + "/12/31";
            ...
            this.sa7abCost=(Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id == c.id_part).First().mcost -
            Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id == c.id_part).First().nowcost)
