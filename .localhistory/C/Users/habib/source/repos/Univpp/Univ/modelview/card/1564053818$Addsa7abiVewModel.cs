         ...
         //   this.date = Ico.getValue<Date>().GetPevDate().year1.Year + "/12/31";
            ...
            foreach (var p in Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id_Pro == pr.Id_Pro).ToList())
            {
                foreach (var m in p.card_mo7sabi.ToList().Where(c => c.visa == null).ToList())
                {
                    ...
                    Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id == m.id_part).ToList().First().mcost -= Cost;
                ...
                dpn+= Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id == p.Id).ToList().First().nowcost ;
                dpm+= Ico.getValue<db>().GetUnivdb().parts.ToList().Where(pr => pr.Id == p.Id).ToList().First().mcost ;

            }
            Ico.getValue<db>().savedb();

            this.sa7abCost = dpm - dpn; 
